...
namespace BelicsClass.File
{
    ...
    public class BL_IniFile
    {
        /// <summary>
        /// コンストラクタです。
        /// 実行ファイルと同階層の(実行ファイル名).iniをiniファイルとして指定します。
        /// </summary>
        public BL_IniFile()
        {
            var fileName = Path.Combine(Path.GetDirectoryName(Application.ExecutablePath),
        ...
        /// <summary>
        /// GetPrivateProfileSectionNamesのDLLインポート<br/>
        /// iniファイルのセクション名取得用の関数(GetPrivateProfileSectionNames)です。<br/>
        /// 本関数の詳細はWin32APIを参照してください。
        /// </summary>
        /// <param name="lpszReturnBuffer">転送先バッファのアドレス</param>
        /// <param name="nSize">転送先バッファのサイズ</param>
        /// <param name="lpFileName">初期化ファイル名のアドレス</param>
        /// <returns></returns>
        ...
        /// <param name="lpReturnedString">転送先バッファのアドレス</param>
        /// <param name="nSize">転送先バッファのサイズ</param>
        /// <param name="lpFileName">初期化ファイル名のアドレス</param>
        /// <returns></returns>
		...
		/// 全てのセクション名を取得します。
		/// </summary>
		/// <returns>セクション名を返します。</returns>
		public string[] GetSectionNames()
        {
            ////読み込みエリア
            string strEntryString = "";

            //全て読み込めるまでトライする
            for (int tryCount = 1; ; ++tryCount)
            {
                //読み込み用のポインタを作成
                IntPtr ptr = Marshal.StringToHGlobalAnsi(new String('\0', readValueSizeCapacity * tryCount));

                //読み込み
                int len = GetPrivateProfileSectionNames(ptr, (uint)(readValueSizeCapacity * tryCount), fileInfo.FullName);

                strEntryString = Marshal.PtrToStringAnsi(ptr, len);

                //ポインタを解放
                Marshal.FreeHGlobal(ptr);

                //全て読み込めた
                if (len + 2 < readValueSizeCapacity * tryCount) break;
            }

            string[] ret = strEntryString.ToString().Trim('\0').Split('\0');
        ...
        /// 全てのキー名、そのキーの値を取得します。
        /// </summary>
        /// <returns>キー名、キーの値を返します(key=value)。</returns>
        public string[] GetKeyNames(string sectionName)
        {
            ...
            for (int tryCount = 1; ; ++tryCount)
            {
                ...
                //読み込み
                int len = GetPrivateProfileSection(sectionName, ptr, (uint)(readValueSizeCapacity * tryCount), fileInfo.FullName);
            ...
            string[] key_values = strEntryString.ToString().Trim('\0').Split('\0');

            string[] ret = new string[key_values.Length];

            for (int i = 0; i < ret.Length; i++)
            {
                var key_value = key_values[i];

                var split = key_value.Split('=');
