...
namespace BelicsClass.UI.Report
{
    ...
    public class BL_ReportView_Base : System.Windows.Forms.UserControl
    {
        ...
        {
            return DuplucatePanel(source, name, row, 0);
        }
        ...
        /// <param name="name">複製後のコントロール名称</param>
        /// <param name="row">複製先の行番号</param>
        /// <param name="source_row">コピー元となる行番号</param>
        /// <returns></returns>
        protected Panel DuplucatePanel(Panel source, string name, int row, int source_row)
        {
            ...
            for (int col = 0; ; col++)
            {
                string key = "R" + source_row.ToString() + "C" + col.ToString();
        ...
        public void PrintAllChildControls(Size docsize, Control root, Point offset, Graphics g, PdfContentByte cb)
        {
            ...
            //foreach (Control control in root.Controls)
            {
                ...
                if (control.Visible)
                {
					...
					if (!printed)
					{
						if (!typeof(BL_ReportTextBox).IsInstanceOfType(control))
						{
