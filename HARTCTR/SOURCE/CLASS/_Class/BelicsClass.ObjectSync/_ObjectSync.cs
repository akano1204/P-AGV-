...
//            FieldInfo[] allfields = type.GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance)
//                .Where(m => Attribute.IsDefined(m, typeof(BL_ObjectSyncAttribute)))
//                .OrderBy(p => (p.GetCustomAttribute(typeof(BL_ObjectSyncAttribute)) as BL_ObjectSyncAttribute).Order).Concat(
//                    type.GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance)
//                    .Where(m => !Attribute.IsDefined(m, typeof(BL_ObjectSyncAttribute)))).ToArray();
            FieldInfo[] allfields = type.GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance)
                .Where(m => Attribute.IsDefined(m, typeof(BL_ObjectSyncAttribute)))
                .OrderBy(p => (p.GetCustomAttributes(typeof(BL_ObjectSyncAttribute), true).First() as BL_ObjectSyncAttribute).Order).Concat(
                    type.GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance)
                    .Where(m => !Attribute.IsDefined(m, typeof(BL_ObjectSyncAttribute)))).ToArray();
            ...
            foreach (var v in allfields)
            {
                ...
                if (type.ToString().IndexOf("System.Collections.Generic.List") == 0 && member_name != "_items") continue;
