...
namespace BelicsClass.UI.Graph
{
    ...
    public partial class BL_Chart : UserControl
    {
        ...
        public class BL_ChartPoint_List : List<BL_ChartPoint>
        {
            BL_ChartAxis xaxis = null;
            BL_ChartAxis yaxis = null;

            public BL_ChartPoint_List()
            {
            }

            public BL_ChartPoint_List(BL_ChartAxis xaxis, BL_ChartAxis yaxis)
            {
                this.xaxis = xaxis;
                this.yaxis = yaxis;
            }

            public new void Add(BL_ChartPoint newpoint)
            {
                BL_ChartPoint addpoint = new BL_ChartPoint();
                if (xaxis != null && xaxis.IsLogarithmic)
                {
                    addpoint.xvalue = Math.Log10(newpoint.xvalue);
                }
                else
                {
                    addpoint.xvalue = newpoint.xvalue;
                }

                if (yaxis != null && yaxis.IsLogarithmic)
                {
                    foreach (var v in newpoint.yvalues)
                    {
                        addpoint.yvalues.Add(Math.Log10(v));
                    }
                }
                else
                {
            ...
            public new void AddRange(IEnumerable<BL_ChartPoint> newpoints)
            {
        ...
        public class BL_ChartSeries
        {
            ...
            public BL_ChartPoint_List Points = null;
            ...
            public BL_ChartSeries(string name, BL_ChartAxis axisX, BL_ChartAxis axisY, Pen border, Brush back, Pen cross, Brush legend)
            {
                ...
                Points = new BL_ChartPoint_List(axisX, axisY);
            }
            ...
            public virtual void DrawValueY(Graphics g, int plotwidth, int width, int height, float positionX)
            {
                ...
                if (0 < yvals.Count)
                {
                    ...
                    Font fontlog = new Font(font.FontFamily, 6f);
                    ...
                    foreach (double y in yvals)
                    {
                        ...
                        if (AxisY.IsLogarithmic) size = g.MeasureString("10", font);
                        ...
                        try
                        {
                            if (typeof(BL_ChartAxis_Left).IsInstanceOfType(AxisY))
                            {
                                ...
                                {
                                    SizeF sz = g.MeasureString(s, fontlog);
                                    RectangleF rect = new RectangleF(AxisY.DisplayWidth + AxisY.DisplayOffset - AxisY.DisplayWidth / 2 - (size.Width + sz.Width) / 2, yy - size.Height / 2, size.Width + sz.Width, size.Height);
                                    g.DrawLine(BorderPen, width, yy, AxisY.DisplayOffset + AxisY.DisplayWidth / 2 + (size.Width + sz.Width) / 2, yy);
                                    g.FillRectangle(BackBrush, rect);
                                    g.DrawRectangle(BorderPen, rect.X, rect.Y, rect.Width, rect.Height);

                                    g.DrawString("10", font, LegendBrush, rect, format);
                                    rect = new RectangleF(AxisY.DisplayWidth + AxisY.DisplayOffset - AxisY.DisplayWidth / 2 + size.Width / 2 - sz.Width + 3, yy - size.Height / 2, sz.Width, sz.Height);
                                    g.DrawString(s, fontlog, LegendBrush, rect, format);
                                }
                                else
                                {
                            ...
                            else if (typeof(BL_ChartAxis_Right).IsInstanceOfType(AxisY))
                            {
                                ...
                                {
                                    SizeF sz = g.MeasureString(s, fontlog);
                                    RectangleF rect = new RectangleF(AxisY.DisplayOffset + AxisY.DisplayWidth / 2 - (size.Width + sz.Width) / 2, yy - size.Height / 2, size.Width + sz.Width, size.Height);
                                    g.DrawLine(BorderPen, 0, yy, AxisY.DisplayOffset + AxisY.DisplayWidth / 2 - size.Width / 2, yy);
                                    g.FillRectangle(BackBrush, rect);
                                    g.DrawRectangle(BorderPen, rect.X, rect.Y, rect.Width, rect.Height);

                                    g.DrawString("10", font, LegendBrush, rect, format);
                                    rect = new RectangleF(AxisY.DisplayOffset + AxisY.DisplayWidth / 2 + size.Width / 2 - sz.Width / 2, yy - size.Height / 2, sz.Width, sz.Height);
                                    g.DrawString(s, fontlog, LegendBrush, rect, format);
                                }
                                else
                                {
            ...
            public virtual void DrawValueX(Graphics g, int width, float positionX)
            {
                ...
                SizeF size = g.MeasureString(s, font);
                if (AxisX.IsLogarithmic) size = g.MeasureString("10", font);
                ...
                if (typeof(BL_ChartAxis_Bottom).IsInstanceOfType(AxisX))
                {
                    try
                    {
                        ...
                        {
                            SizeF sz = g.MeasureString(s, fontlog);
                            RectangleF rect = new RectangleF(xx - (size.Width + sz.Width) / 2, AxisX.DisplayOffset + AxisX.DisplayWidth / 2 - size.Height / 2, size.Width + sz.Width + 3, size.Height);
                            g.DrawLine(BorderPen, xx, 0, xx, AxisX.DisplayOffset + AxisX.DisplayWidth / 2 - size.Height / 2);
                            g.FillRectangle(BackBrush, rect);
                            g.DrawRectangle(BorderPen, rect.X, rect.Y, rect.Width, rect.Height);

                            g.DrawString("10", font, LegendBrush, rect, format);
                            rect = new RectangleF(xx + size.Width / 2, AxisX.DisplayOffset + AxisX.DisplayWidth / 2 - size.Height / 2, sz.Width, sz.Height);
                            g.DrawString(s, fontlog, LegendBrush, rect, format);
                        }
                        else
                        {
		...
		{
			/// <summary></summary>
			public float PointDistance = float.NaN;
			...
			public BL_ChartSeries_Line(string name, BL_ChartAxis axisX, BL_ChartAxis axisY, Pen border, Brush back, Pen cross, Brush legend, float point_distance)
				: base(name, axisX, axisY, border, back, cross, legend)
			{
				...
				foreach (BL_ChartPoint point in Points)
				{
					try
					{
						...
						if (!bfirst)
						{
							...
							{
								if (PointDistance != double.NaN) distance = Math.Sqrt((xpre - x) * (xpre - x) + (ypre - y) * (ypre - y));

								if (PointDistance != double.NaN && distance <= PointDistance)
								{
									if (!plot.ContainsKey((int)x)) plot[(int)x] = new Dictionary<int, byte>();

									if (!plot[(int)x].ContainsKey((int)y))
									{
										if (hdc == IntPtr.Zero) hdc = g.GetHdc();

										GDI.SetPixel(hdc, (int)xpre, (int)ypre, colorRef);
										plot[(int)x][(int)y] = 1;
									}
								}
								else if ((int)x != (int)xint)
								{
									if (hdc != IntPtr.Zero)
									{
										g.ReleaseHdc(hdc);
        ...
        public class BL_ChartSeries_Bar : BL_ChartSeries
        {
            ...
            public BL_ChartSeries_Bar(string name, BL_ChartAxis axisX, BL_ChartAxis axisY, Pen border, Brush back, Pen cross, Brush legend, double barwidth, double baroffset, params Brush[] secondary_backs)
                : base(name, axisX, axisY, border, back, cross, legend)
            {
                BarWidth = barwidth;
				BarOffset = baroffset;
			...
			public BL_ChartSeries_Bar(string name, BL_ChartAxis axisX, BL_ChartAxis axisY, Pen border, Brush back, Pen cross, Brush legend, double barwidth, params Brush[] secondary_backs)
				: this(name, axisX, axisY, border, back, cross, legend, barwidth, 0, secondary_backs)
			{
			}
            ...
            public override void Draw(Graphics g, int width, int height)
            {
                ...
                try
                {
                    ...
                    {
                        double fromy = 0;
						float x1 = AxisX.GetPixel(width, point.xvalue + (float)(BarOffset * BarWidth));
						float x2 = AxisX.GetPixel(width, point.xvalue + (float)(BarOffset * BarWidth) + BarWidth);
		...
		/// 棒
		/// </summary>
		public class BL_ChartSeries_Bar_TimeSpan : BL_ChartSeries_Bar
		{
			/// <summary>
			/// 
			/// </summary>
			...
			/// <param name="barwidth"></param>
			/// <param name="secondary_backs"></param>
			public BL_ChartSeries_Bar_TimeSpan(string name, BL_ChartAxis axisX, BL_ChartAxis axisY, Pen border, Brush back, Pen cross, Brush legend, double barwidth, double baroffset, params Brush[] secondary_backs)
				: base(name, axisX, axisY, border, back, cross, legend, barwidth, baroffset, secondary_backs)
			{
			}

			public BL_ChartSeries_Bar_TimeSpan(string name, BL_ChartAxis axisX, BL_ChartAxis axisY, Pen border, Brush back, Pen cross, Brush legend, double barwidth, params Brush[] secondary_backs)
				: this(name, axisX, axisX, border, back, cross, legend, barwidth, 0, secondary_backs)
			{
			}
			...
			public override void Draw(Graphics g, int width, int height)
			{
				...
				Font font = new Font(AxisY.Owner.Font.Name, AxisY.Owner.Font.Height / 2);
				float size = AxisX.GetPixelSize(width, BarWidth);
				...
				try
				{
					foreach (BL_ChartPoint point in Points)
					{
						...
						foreach (double yval in point.yvalues)
						{
							...
							string s = AxisY.ToString(yval);
							SizeF sizef = g.MeasureString(s, font);

							g.DrawString(s, font, BorderPen.Brush, x1 - sizef.Width / 2, y1 - sizef.Height);
            ...
            public override void Draw(Graphics g, int width)
            {
                ...
                if (min < 0 && 0 < max && 0 != GuideOuter)
                {
                    for (double x = GuideOffset; min <= x; x -= GuideOuter)
                    {
                        ...
                        string s = ToString(x);
                        SizeF size = g.MeasureString(s, font);
                        if (IsLogarithmic) size = g.MeasureString("10", font);

                        format.FormatFlags = 0;
						if (width / Maximum / GuideOuter < this.DisplayWidth)
						{
							format.Alignment = StringAlignment.Near;
							format.FormatFlags |= StringFormatFlags.DirectionVertical;
							size = new SizeF(size.Height, DisplayWidth);
						}
                            ...
                            ((xpre == 0 || xpre < xx) && RightToLeft))
                        {
                            ...
                            if (IsLogarithmic)
                            {
                                g.DrawString("10", font, DrawPen.Brush, new RectangleF(xx, DisplayOffset + 12.0f, size.Width, size.Height), format);
                                SizeF sz = g.MeasureString("10", font);
                                g.DrawString(s, fontlog, DrawPen.Brush, new RectangleF(xx, DisplayOffset + 12.0f, size.Width + sz.Width + 3, size.Height), format);
                            }
                            else
                            {
		...
		public class BL_ChartAxis_Bottom_Text : BL_ChartAxis_Bottom
		{
			public Dictionary<double, string> dictTexts = new Dictionary<double, string>();
            ...
            /// <param name="rightmargin"></param>
            /// <param name="rate"></param>
            public BL_ChartAxis_Bottom_Text(Control owner, string name, double min, double max, double guideinner, double guideouter, double guideoffset, int displaywidth, int displayoffset, bool righttoleft, string format, Pen draw, int fontmargin, int leftmargin, int rightmargin, double rate)
                : this(owner, name, min, max, guideinner, guideouter, guideoffset, displaywidth, displayoffset, righttoleft, format, draw, fontmargin, leftmargin, rightmargin, rate, 0)
            {
            }
            ...
            /// <param name="rate"></param>
            /// <param name="continue_threshold"></param>
            public BL_ChartAxis_Bottom_Text(Control owner, string name, double min, double max, double guideinner, double guideouter, double guideoffset, int displaywidth, int displayoffset, bool righttoleft, string format, Pen draw, int fontmargin, int leftmargin, int rightmargin, double rate, double continue_threshold)
                : base(owner, name, min, max, guideinner, guideouter, guideoffset, displaywidth, displayoffset, righttoleft, format, draw, fontmargin, leftmargin, rightmargin, rate, continue_threshold)
            {
            }
			...
			public override string ToString(double value)
			{
				if (dictTexts.ContainsKey(value))
				{
					return dictTexts[value];
				}

				double pre = double.NaN;
				foreach (var v in dictTexts)
				{
					if (double.IsNaN(pre))
					{
						//continue;
						//if (value < v.Key) return v.Value;
					}
					else
					{
						if (pre < value && value < v.Key)
						{
							return v.Value;
						}
					}
			...
			public override void Draw(Graphics g, int width)
			{
				...
				double min = (long)Minimum;
				double max = (long)Maximum;
				if (Maximum < Minimum) { min = Maximum; max = Minimum; }
                    ...
                    if (GuideOuter != 0)
                    {
                        for (double y = GuideOffset; min <= y; y -= GuideOuter)
                        {
                                ...
                                ((ypre == 0 || ypre < yy) && RightToLeft))
                            {
                                ...
                                if (IsLogarithmic)
                                {
                                    SizeF szz = g.MeasureString(s, fontlog);
                                    g.DrawString("10", font, DrawPen.Brush, new RectangleF(DisplayWidth - (size.Width + szz.Width) - 12 + DisplayOffset, yy, size.Width, size.Height), format);
                                    g.DrawString(s, fontlog, DrawPen.Brush, new RectangleF(DisplayWidth - szz.Width - 12 + DisplayOffset, yy, szz.Width, szz.Height), format);
                                }
                                else
                                {
		...
		public class BL_ChartAxis_Left_DateTime : BL_ChartAxis_Left
		{
			/// <summary>
            /// 
            /// </summary>
            ...
            /// <param name="bottommargin"></param>
            /// <param name="rate"></param>
			public BL_ChartAxis_Left_DateTime(Control owner, string name, double min, double max, double guideinner, double guideouter, double guideoffset, int displaywidth, int displayoffset, bool bottomtoup, string format, Pen draw, int fontmargin, int topmargin, int bottommargin, double rate)
				: base(owner, name, min, max, guideinner, guideouter, guideoffset, displaywidth, displayoffset, bottomtoup, format, draw, fontmargin, topmargin, bottommargin, rate)
			{
			}
			...
			public override string ToString(double value)
			{
				try
				{
					...
					//return string.Format(IndicateFormat, ts);

					string ret = IndicateFormat;

					ret = ret.Replace("Td", ((int)ts.TotalDays).ToString());
					ret = ret.Replace("dddd", ts.Days.ToString("".PadRight(4, '0')));
					ret = ret.Replace("ddd", ts.Days.ToString("".PadRight(3, '0')));
					ret = ret.Replace("dd", ts.Days.ToString("".PadRight(2, '0')));
					ret = ret.Replace("d", ts.Days.ToString());

					ret = ret.Replace("Th", ((int)ts.TotalHours).ToString());
					ret = ret.Replace("hhh", ts.Hours.ToString("".PadRight(3, '0')));
					ret = ret.Replace("hh", ts.Hours.ToString("".PadRight(2, '0')));
					ret = ret.Replace("h", ts.Hours.ToString());

					ret = ret.Replace("Tm", ((int)ts.TotalMinutes).ToString());
					ret = ret.Replace("mmm", ts.Minutes.ToString("".PadRight(3, '0')));
					ret = ret.Replace("mm", ts.Minutes.ToString("".PadRight(2, '0')));
					ret = ret.Replace("m", ts.Minutes.ToString());

					ret = ret.Replace("Ts", ((int)ts.TotalSeconds).ToString());
					ret = ret.Replace("sss", ts.Seconds.ToString("".PadRight(3, '0')));
					ret = ret.Replace("ss", ts.Seconds.ToString("".PadRight(2, '0')));
					ret = ret.Replace("s", ts.Seconds.ToString());

					ret = ret.Replace("Tf", ts.TotalMilliseconds.ToString());
					ret = ret.Replace("fff", ts.Milliseconds.ToString("".PadRight(3, '0')));
					ret = ret.Replace("ff", ts.Milliseconds.ToString("".PadRight(2, '0')));
					ret = ret.Replace("f", ts.Milliseconds.ToString());
                ...
                if (min < 0 && 0 < max && GuideOuter != 0)
                {
                    for (double y = GuideOffset; min <= y; y -= GuideOuter)
                    {
                            ...
                            ((ypre == 0 || ypre < yy) && RightToLeft))
                        {
                            ...
                            if (IsLogarithmic)
                            {
                                g.DrawString("10", font, DrawPen.Brush, new RectangleF(12 + DisplayOffset, yy, size.Width, size.Height), format);
                                SizeF szz = g.MeasureString(s, fontlog);
                                g.DrawString(s, fontlog, DrawPen.Brush, new RectangleF(12 + DisplayOffset + size.Width, yy, szz.Width, szz.Height), format);
                            }
                            else
                            {
		...
		public bool EnableScroll { get { return _enablescroll; } set { _enablescroll = value; } }
        ...
        public void RecalculateAxesScale(double min, double max)
        {
            ...
            foreach (var v in minmax)
            {
                ...
                if (v.Value[0] != 0.0 && v.Value[0] != double.MaxValue)
                {
                    ...
                    {
						double multi = Math.Abs((long)(1.0 / diff));
                    ...
                    {
						double multi = Math.Abs((long)v.Value[0]);
                        int power = multi.ToString().Length;
                ...
                if (v.Value[1] != 0.0 && v.Value[1] != double.MinValue)
                {
                    ...
                    {
                        double multi = Math.Abs((long)v.Value[1]);
                        int power = multi.ToString().Length;
                        int pvalue = (int)Math.Pow(10, power - 1);
