...
namespace BelicsClass.PLC
{
	...
	/// </summary>
	public class BL_PLCEngine_Base : BL_ThreadController_Base
	{
			...
			public T Get<T>(int position)
			{
				...
				else
				{
					...
					switch (valueType)
					{
						...
						case ValueType.STRING: return (T)((object)Encoding.Default.GetString(bigdata).Substring(position).TrimEnd('\0'));
				...
				if (position < 0)
				{
					...
					else
					{
						for (int i = 0; i < (temp.Length + 1) / 2; i++)
						{
							...
							{
								data[i * 2 + 0] = temp[i * 2 + 1];
							}
							else data[i * 2 + 0] = 0;

							if (i * 2 + 1 < data.Length)
							{
								data[i * 2 + 1] = temp[i * 2 + 0];
							}
						}
					}
				}
		...
		public delegate void Event_Handler_ValueChanged(BL_PLCEngine_Base plc, DeviceInfo deviceinfo);
		...
		public delegate void Event_Handler_PlcError(BL_PLCEngine_Base plc, bool error, string errordescription);
		...
		public BL_PLCEngine_Base(BL_Rs232c.PortNo portNo, BL_Rs232c.BaudRate baudRate, BL_Rs232c.ByteSize byteSize, BL_Rs232c.Parity parity, BL_Rs232c.StopBits stopBits, BL_Rs232c.FormatType formatType)
		{
		...
		public BL_PLCEngine_Base(bool isUDP, string ipaddress, int portno)
		{
			if (!isUDP)
			{
		...
		public BL_PLCEngine_Base(bool isUDP, string ipaddress, int portno, bool SupportRandomRead)
		...
		/// <param name="ipaddress">IPアドレス</param>
		/// <param name="portno">ソケットポートNo</param>
		public BL_PLCEngine_Base(string ipaddress, int portno)
		...
		/// <param name="portno">ソケットポートNo</param>
		/// <param name="SupportRandomRead">複数ブロック一括読み出し可能</param>
		public BL_PLCEngine_Base(string ipaddress, int portno, bool SupportRandomRead)
			: this(false, ipaddress, portno, SupportRandomRead)
		{
		}
					...
					#region PLC接続
					{
						switch (DeviceType)
						{
							...
							case DeviceTypes.UDP:
								...
								SocketUDP = new BL_RawSocketUDP(portno);

								break;

							case DeviceTypes.RS232C:
								if (Com != null) if (Com.IsOpened) Com.Close();
								Com = new BL_Rs232c();
								break;
						}
					...
					#region 読み出し要求コマンド送信(複数ブロック一括読み出し)
					{
						...
						lock (queRead)
						{
							while (0 < queRead.Count)
							{
								...
								{
									if (0 < r_bitdevices.Count || 0 < r_worddevices.Count) break;
								}
							}
						}
						...
						foreach (var v in r_bitdevices)
						{
							...
							{
								address = 0;
								for (int i = 0; i < v.deviceAddress; i++, address++)
								{
									if (i % 100 == 15) i += 84;
								}
							}
				...
				case 110:
					#region 読み出し応答コマンド受信(複数ブロック一括読み出し)
					{
						...
						while (11 <= ReceiveBytes.Length)
						{
							...
							{
								ReceiveBytes = BL_Bytes.Trim(ReceiveBytes, 2, ReceiveBytes.Length - 2);
							}
						}

						if (11 <= ReceiveBytes.Length)
						{
							int length = ReceiveBytes[7] + ReceiveBytes[8] * 0x100 + 9;

							if (length <= ReceiveBytes.Length)
							{
								data = BL_Bytes.Trim(ReceiveBytes, length);
								ReceiveBytes = new byte[0];
									...
									if (11 <= ReceiveBytes.Length)
									{
										...
										if (length <= ReceiveBytes.Length)
										{
											...
											if (data[9] == 0 && data[10] == 0)
											{
												...
												else if (0 < queWrite.Count) m_Step = 400;
												else m_Step = 100;
											}
											else
											{
