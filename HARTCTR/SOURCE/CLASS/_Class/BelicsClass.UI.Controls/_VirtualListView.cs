...
namespace BelicsClass.UI.Controls
{
	...
	public class BL_VirtualListView : BL_DoubleBufferListView
	{
        ...
        private Color titleBackColor = SystemColors.Control;
        ...
        /// </summary>
        /// <param name="index"></param>
        public new void EnsureVisible(int index)
        {
            BL_VirtualListViewItem lvi = Items[index];
            EnsureVisible(lvi);
        }
		...
		private void sort_item()
		{
			...
			foreach (var v in Items)
			{
				if (0 <= c)
				{
					string n = v[c].ToString().Replace(",", "").Replace("\\", "").Replace(@"¥", "");
					if (!double.TryParse(n, out num)) isnum = false;
				}
				SortedItems.Add(v);
			}
			...
			if (0 <= c)
			{
				...
				{
                    if (isnum) SortedItems.Sort(delegate (BL_VirtualListViewItem mca1, BL_VirtualListViewItem mca2)
                    {
                        if (double.Parse(mca1[c].ToString().Replace(",", "").Replace("\\", "")) == double.Parse(mca2[c].ToString().Replace(",", "").Replace("\\", "").Replace(@"¥", "")))
                        {
                            return 0;
                        }
                        else if (double.Parse(mca1[c].ToString().Replace(",", "").Replace("\\", "")) > double.Parse(mca2[c].ToString().Replace(",", "").Replace("\\", "").Replace(@"¥", "")))
                        {
                            return 1;
                        }
                        else
                    ...
                    else SortedItems.Sort(delegate (BL_VirtualListViewItem mca1, BL_VirtualListViewItem mca2)
                    {
                        return string.Compare(mca1[c].ToString(), mca2[c].ToString());
                    });
				}
				else if (this.Sorting == SortOrder.Descending)
				{
					if (isnum) SortedItems.Sort(delegate(BL_VirtualListViewItem mca1, BL_VirtualListViewItem mca2)
                    {
                        ...
                        else if (double.Parse(mca2[c].ToString().Replace(",", "").Replace("\\", "")) > double.Parse(mca1[c].ToString().Replace(",", "").Replace("\\", "").Replace(@"¥", "")))
                        {
                            return 1;
                        }
                        else
                    ...
                    {
                        return string.Compare(mca2[c].ToString(), mca1[c].ToString());
                    });
				}
			}
		...
		protected override void OnSelectedIndexChanged(EventArgs e)
		{
			...
			for (int i = 0; i < m_Sel.Count; i++)
			{
                ...
                base.Items[m_Sel[i]].Focused = true;
                base.EnsureVisible(m_Sel[i]);
		...
		protected override void OnDrawColumnHeader(DrawListViewColumnHeaderEventArgs e)
		{
            ...
            SolidBrush br_b = new SolidBrush(TitleBackColor);
            SolidBrush br_f = new SolidBrush(TitleForeColor);
            Pen pen = new Pen(br_f, 1);
			...
			{
				e.Graphics.FillRectangle(br_b, e.Bounds);
				e.Graphics.DrawRectangle(pen, e.Bounds);

				if (this.Sorting == SortOrder.Ascending)
				{
					e.Graphics.DrawString("▴" + e.Header.Text, e.Font, br_f, e.Bounds, sf);
