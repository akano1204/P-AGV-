...
namespace BelicsClass.Network
{
    ...
    public class BL_FtpClient
    {
        ...
        private string MsgFromServer = "";					// サーバからのメッセージ 保持フィールド
        private NetworkStream netStream_Ctrl = null;		// コントロールコネクション用 ストリームクラス
        private BL_HostInfo hostInfo = new BL_HostInfo();			// ホスト（サーバ）情報

        private bool isLogin = false;						// ログイン状態
        private string RemotePath = "\\";					// サーバのルートディレクトリ指定用

        private BL_FtpErrorInfo m_LastError;					// FTPエラー情報
        ...
        // サーバのルートディレクトリにあるFileList
        private StringCollection sc = new StringCollection();
        // Fileリストに対応した各ファイルサイズ
        private StringCollection m_ScFileSize = new StringCollection();
        // Fileリストに対応した各ファイルの日付
        private StringCollection m_ScFileDate = new StringCollection();
        #endregion
        ...
        // ホストへのコマンド送信メソッド
        //////////////////////////////////////////////
        private bool SendToServer(string command)
        {
            string msg = command + "\r\n";
        ...
        public bool Login(string remotePath)
        {
        ...
        /// <returns>成功/失敗：true/false</returns>
        public bool DeleteFile(string targetFilePath)
        {
            int check = 0;
        ...
        // サーバからのメッセージ入力
        //////////////////////////////////////////////
        private int GetMessage(int waitTime)
        {
            int Code = 0;
            int LastCode = 0;
            string temp = "";
        ...
        // エラー情報格納サブルーチン
        //////////////////////////////////////////////
        private void SetLastError(int code, int answer, string message)
        {
        ...
        /// </summary>
        /// <returns>成功／失敗：true / false</returns>
        public bool RefreshFileList(string targetPath)
        {
            int check = 0;
